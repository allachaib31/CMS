...
function calcNodeXPosition(spacing, nodeWidth, isSmall) {
    if (isSmall) {
...
export function positionUnknownGeneration(otherGenerationNodes) {
...
function sumParentsSize(couple, familyMembers) {
    return couple.parents
        .map((parent) => calcNodeXPosition(SPACING, NODE_WIDTH, familyMembers[parent].data.isHidden))
        .reduce((acc, cur) => acc + cur, 0);
}

export function positionAndBuildFamilyTree(innerFamiliesPerGeneration, familyMembers) {
    const familySizeRecurse = (couple) => {
    ...
    const familyCenterPositionRecurse = (couple) => {
    ...
    const familyTreeCreateNodesRecurse = (couple) => {
        ...
        const positionedNodesOfCouples = parentsFamilyMembers.map((parent) => {
...
function buildNodeFromFamilyMember(parent, x, y) {
    return {
        id: parent.id,
        data: {
            ...parent.data
...
export function addNodeVisibilityCallback(nodes, hiddenNodeIds, setHiddenNodeIds) {
...
export function addNodeSelection(familyRelations, nodes, selectedNodeId) {
    return nodes.map((node) => {
