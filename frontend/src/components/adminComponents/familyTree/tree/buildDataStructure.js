...
function tryInferGenerationForOthers(
    ...
    const others = familyMemberByGeneration
        .filter(([gen]) => gen == OTHERS_GENERATION)
        .map(([, member]) => member);
    const zeroGen = familyMemberByGeneration.filter(([gen]) => gen == 0).map(([, member]) => member);
...
export function buildGenerations(
    ...
    const familyMembersByGeneration = Object.values(familyMembers).map((member) => {
        ...
        if (relationToRoot) {
            const generation = getGenerationFromRelation(relationToRoot);
...
function buildCouplesPerGeneration(
    ...
    const couplesPerGeneration = Object.fromEntries(
        Object.entries(familyGenerations).map(([rawGeneration, nodesInGeneration]) => {
            const generation = parseInt(rawGeneration);
            const couples = [];
            ...
            while (availableNodesInGeneration.length > 0) {
                ...
                const partnersNodes = partnersIds
                    .map((partnerId) => {
...
export function buildParentsChildrenStructs(familyMembers, familyRelations) {
