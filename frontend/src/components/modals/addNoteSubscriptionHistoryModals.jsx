...
import { updateCommentSubscribeHistoryFetch } from "../../utils/apiFetch";
function AddNoteModals({ inputs, setInputs }) {
    const navigate = useNavigate();
    const [submit, setSubmit] = useState(false);
    const handleSubmit = () => {
        setSubmit((e) => !e);
        updateCommentSubscribeHistoryFetch(inputs).then((res) => {
            setSubmit((e) => !e);
    ...
    return (
        <dialog id="addNote" className="modal">
            <div className="modal-box">
                <h3 className="font-bold text-lg">اضف ملاحظة لي {inputs.name}</h3>
