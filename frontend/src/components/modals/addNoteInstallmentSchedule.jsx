...
import { addCommentInstallmentScheduleFetch } from '../../utils/apiFetch';

function AddNoteInstallmentSchedule({ inputs, setInputs, installmentSchedule, setInstallmentSchedule }) {
    const navigate = useNavigate();
    const [submit, setSubmit] = useState(false);
    const handleSubmit = () => {
        setSubmit((e) => !e);
        addCommentInstallmentScheduleFetch(inputs).then((res) => {
            setSubmit((e) => !e);
            const newInstallment = installmentSchedule;
            newInstallment[inputs.index].comments = inputs.comment
    ...
    return (
        <dialog id="addNote" className="modal">
            <div className="modal-box">
                <h3 className="font-bold text-lg">اضف ملاحظة لي {inputs._id}</h3>
...
export default AddNoteInstallmentSchedule