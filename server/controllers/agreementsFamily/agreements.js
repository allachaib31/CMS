const { validateAgreement, agreementsFamilyModel } = require("../../models/agreementsFamily/agreements");
...
exports.addAgreements = async (req, res) => {
    ...
    try {
        ...
        const agreements = new agreementsFamilyModel({
            title,
            text,hijriDate: {
                day: hijriDate[0],
                month: hijriDate[1],
...
exports.getAgreements = async (req, res) => {
    try {
        const agreements = await agreementsFamilyModel.find();
        return res.status(200).send({
            agreements
        })
    } catch (error) {
...
exports.updateAgreementsActive = async (req, res) => {
    ...
    try {
        ...
        if(!active) {
            const findActiveAgreement = await agreementsFamilyModel.findOne({
                active: true
            });
        ...
        const agreements = await agreementsFamilyModel.findByIdAndUpdate(id, {
            active: !active
        });
        const result = await agreementsFamilyModel.find().sort({
            createdAt: -1
        });
        return res.status(200).send({
            agreements,
